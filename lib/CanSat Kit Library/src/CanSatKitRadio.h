...
/**
 * @brief CansatFrame object represents one radio frame to be send over the RF link.
 * Use it the same as Serial/SerialUSB devices.
 * CansatFrame should consist of ASCII-characters only.
 ...
 */
class CansatFrame : public Print {
 public:
  CansatFrame() : size(0) {}
...
class Radio {
 public:
   ...
   */
  static bool transmit(CansatFrame frame);
